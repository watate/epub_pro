...
void main() {
  group('BlackSwan Duplicate TOC Tests', () {
    ...
    test('should not have duplicate TOC entries for same file with different anchors', () async {
      ...
      expect(contentFileNames.length, equals(uniqueContentFiles.length),
          reason: 'Found duplicate content files in TOC: ${contentFileNames.where((file) => contentFileNames.where((f) => f == file).length > 1).toSet()}');
      
      // Specifically check for the problematic file from blackSwan.epub
      final part0004Files = contentFileNames.where((file) => file.contains('part0004_split_000.html')).toList();
      expect(part0004Files.length, equals(1),
          reason: 'part0004_split_000.html should appear only once in TOC, but found ${part0004Files.length} times');
    });

    test('should maintain proper chapter hierarchy despite skipping duplicates', () async {
      // Check if blackSwan.epub exists
      final blackSwanPath = path.join(
        io.Directory.current.path,
        'assets',
      ...
      final prologueChapter = chapters.where((ch) => (ch.title?.contains('Prologue') ?? false)).toList();
      ...
      expect(prologueChapter.first.contentFileName, equals('text/part0004_split_000.html'));
    });
  });
}