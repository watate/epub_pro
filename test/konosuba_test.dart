  ...
  group('KonoSuba Japanese EPUB Tests', () {
    ...
    setUpAll(() async {
      ...
      if (!(await konosubaFile.exists())) {
        print('Skipping KonoSuba tests - konosuba.epub not found at: $konosubaPath');
        return;
      }
    ...
    test('handles minimal navigation with spine reconciliation', () async {
      ...
      expect(navPoints.first.navigationLabels!.first.text, equals('表紙')); // "Cover"
    ...
    test('correctly identifies all 10 chapters from spine', () async {
      ...
      for (var i = 0; i < chapters.length; i++) {
        ...
        expect(chapter.title, isNotEmpty, reason: 'Chapter titles should not be empty');
    ...
    test('has correct Japanese metadata', () async {
      ...
      // Check language
      expect(konosubaBook.schema?.package?.metadata?.languages.first, equals('ja'));
      
      // Check publisher
      expect(konosubaBook.schema?.package?.metadata?.publishers.first, equals('角川書店'));
      
      if (verbose) {
        print('\n=== Metadata ===');
        print('Title: ${konosubaBook.title}');
        print('Author: ${konosubaBook.author}');
        print('Language: ${konosubaBook.schema?.package?.metadata?.languages.first}');
        print('Publisher: ${konosubaBook.schema?.package?.metadata?.publishers.first}');
        print('Publication Date: ${konosubaBook.schema?.package?.metadata?.dates.firstOrNull?.date}');
      }
    });
    ...
    test('has right-to-left page progression', () async {
      ...
      if (verbose) {
        ...
        print('Primary writing mode: ${konosubaBook.schema?.package?.metadata?.metaItems.firstWhere(
    ...
    test('handles cover image correctly', () async {
      ...
      final coverManifestItem = konosubaBook.schema?.package?.manifest?.items.firstWhere(
        (item) => item.id == 'cover',
      );
    ...
    test('word counting should work correctly for Japanese text', () async {
      ...
      final japaneseChapter = konosubaBook.chapters.firstWhere(
        ...
        orElse: () => konosubaBook.chapters[2], // fallback to any content chapter
      );
    ...
    test('eager splitting should work for Japanese chapters', () async {
      ...
      final konosubaBytes = await konosubaFile.readAsBytes();
      final splitBook = await EpubReader.readBookWithSplitChapters(konosubaBytes);
    ...
    test('counts total words in the book', () async {
      ...
      for (var i = 0; i < konosubaBook.chapters.length; i++) {
        ...
        if (verbose) {
          ...
          print('$paddedIndex | $paddedWords | $paddedChars | ${chapter.title ?? "[Untitled]"}');
        }
      }
      ...
      if (verbose) {
        ...
        print('Total characters: ${totalCharacters.toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}');
        print('Average words per chapter: ${(totalWords / konosubaBook.chapters.length).round()}');
        print('Average characters per chapter: ${(totalCharacters / konosubaBook.chapters.length).round()}');
        print('Characters to words ratio: ${(totalCharacters / totalWords).toStringAsFixed(2)}');
        ...
        if (chapterStats.isNotEmpty) {
          ...
          for (var i = 0; i < 3 && i < chapterStats.length; i++) {
            ...
            print('${i + 1}. ${stat['title']} - ${stat['words']} words (${stat['characters']} chars)');
          }

          print('\n=== Shortest Chapters by Word Count ===');
          final nonZeroChapters = chapterStats.where((s) => s['words'] > 0).toList();
      ...
      expect(totalCharacters, greaterThan(totalWords), 
          reason: 'Japanese text typically has more characters than word count');
