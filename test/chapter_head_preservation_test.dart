...
void main() {
  group('Chapter Head Preservation Tests', () {
    ...
    test('original chapters contain head elements', () async {
        ...
        try {
          ...
          for (final chapter in book.chapters) {
            if (chapter.htmlContent != null && chapter.htmlContent!.isNotEmpty) {
              ...
              final headPattern = RegExp(r'<head[^>]*>.*?</head>', dotAll: true);
      ...
      print('Found $foundChaptersWithHeads chapters with <head> out of $totalChaptersChecked total chapters');
      expect(foundChaptersWithHeads, greaterThan(0),
          reason: 'Expected to find at least some chapters with <head> elements');
    });
    ...
    test('split chapters retain head elements', () async {
      ...
      print('Found $foundSplitChaptersWithHeads split chapters with <head> out of $totalSplitChaptersChecked total split chapters');
      
      if (!foundSplitChapters) {
        print('No split chapters found - some EPUBs may not have chapters long enough to trigger splitting');
        return;
      }

      // This test is expected to fail, demonstrating the bug
      expect(foundSplitChaptersWithHeads, equals(totalSplitChaptersChecked),
          reason: 'Split chapters should retain their <head> elements but currently do not');
    });
    ...
    test('split chapters preserve title elements within head', () async {
          ...
          for (final chapter in splitBook.chapters) {
            if (chapter.htmlContent != null && chapter.htmlContent!.isNotEmpty) {
              ...
              if (chapter.title != null && 
                ...
                final titlePattern = RegExp(r'<head[^>]*>.*?<title[^>]*>.*?</title>.*?</head>', dotAll: true);
      ...
      print('Found $foundSplitChaptersWithTitles split chapters with <title> in <head> out of $totalSplitChaptersChecked total split chapters');
      ...
      expect(foundSplitChaptersWithTitles, greaterThan(0),
          reason: 'Split chapters should preserve <title> elements within <head> but currently do not');
    });
    ...
    test('split chapters preserve CSS links in head', () async {
          ...
          for (final chapter in splitBook.chapters) {
            if (chapter.htmlContent != null && chapter.htmlContent!.isNotEmpty) {
              ...
              if (chapter.title != null && 
                ...
                final cssPattern = RegExp(r'<head[^>]*>.*?<link[^>]*rel=["\x27]stylesheet["\x27][^>]*>.*?</head>', dotAll: true);
      ...
      print('Found $foundSplitChaptersWithCSS split chapters with CSS links in <head> out of $totalSplitChaptersChecked total split chapters');
      ...
      expect(foundSplitChaptersWithCSS, greaterThan(0),
          reason: 'Split chapters should preserve CSS link elements within <head> but currently do not');
    });
  });
}