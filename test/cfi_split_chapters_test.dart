  ...
  group('CFI Split Chapters Compatibility Tests', () {
    ...
    setUpAll(() async {
      ...
      if (!frankensteinFile.existsSync()) {
        throw StateError(
            'Frankenstein EPUB not found at assets/frankenstein.epub');
      }
      epubBytes = await frankensteinFile.readAsBytes();
      print(
          '📚 Loaded Frankenstein EPUB (${epubBytes.length} bytes) for split chapter CFI testing');
    });
    ...
    group('📊 Split Chapter Structure Analysis', () {
      ...
      test('Compare Chapter Structure: Normal vs Split', () async {
        ...
        final splitChapterRefs =
            await splitBookRef.getChapterRefsWithSplitting();
        ...
        for (final chapterRef in splitChapterRefs) {
          if (chapterRef is EpubChapterSplitRef) {
            splitPartCount++;
            print(
                '  Split Part: "${chapterRef.title}" (${chapterRef.partNumber}/${chapterRef.totalParts})');
            ...
            expect(chapterRef.partNumber,
                lessThanOrEqualTo(chapterRef.totalParts));
            expect(chapterRef.originalChapter, isNotNull);
          } else {
            originalChapterCount++;
            print(
                '  Normal: "${chapterRef.title ?? chapterRef.contentFileName}"');
          }
        }
        ...
        // Basic validation
        expect(splitPartCount + originalChapterCount,
            equals(splitChapterRefs.length));
        expect(
            splitPartCount, greaterThan(0)); // Should have some split chapters
        expect(splitChapterRefs.length,
            greaterThanOrEqualTo(normalChapters.length));
      });
      ...
      test('Spine Mapping Consistency Between Normal and Split', () async {
        ...
        for (int i = 0; i < 3 && i < normalSpineMap.length; i++) {
          ...
          if (normalChapter != null && splitChapter != null) {
            print(
                '  Spine $i: "${normalChapter.contentFileName}" -> "${splitChapter.contentFileName}"');
            expect(splitChapter.contentFileName,
                equals(normalChapter.contentFileName));
          }
        }
      });
    });
    ...
    group('🎯 CFI Generation with Split Chapters', () {
      ...
      test('Progress CFI Generation - Normal vs Split', () async {
        ...
        for (int spineIndex = 0;
            spineIndex < 3 && spineIndex < normalBookRef.spineItemCount;
            spineIndex++) {
          ...
          expect(splitCFI.structure.start.parts.length,
              equals(normalCFI.structure.start.parts.length));

          // Test fractional progress CFIs
          final normalFractional =
              normalBookRef.createProgressCFI(spineIndex, fraction: 0.5);
          final splitFractional =
              splitBookRef.createProgressCFI(spineIndex, fraction: 0.5);

          print('    Normal 50%: ${normalFractional.toString()}');
          print('    Split 50%:  ${splitFractional.toString()}');

          expect(
              splitFractional.toString(), equals(normalFractional.toString()));
        }
      });
      ...
      test('CFI Navigation - Split Chapter Compatibility', () async {
        ...
        for (int i = 0; i < testCFIs.length; i++) {
          ...
          print(
              '    Normal navigation: ${normalSuccess ? "Success" : "Failed"}');
          print(
              '    Split navigation:  ${splitSuccess ? "Success" : "Failed"}');

          if (normalSuccess && splitSuccess) {
            // Compare spine indices
            expect(
                splitLocation!.spineIndex, equals(normalLocation!.spineIndex));
            print('    Both found in spine: ${normalLocation.spineIndex}');

            // Content file should be the same
            expect(splitLocation.chapterRef.contentFileName,
          ...
          if (normalSuccess) {
            expect(splitSuccess, isTrue,
                reason:
                    'Split book should handle CFI navigation at least as well as normal book');
          }
        }
      });
      ...
      test('CFI Generation from Split Chapter References', () async {
        ...
        final splitChapters =
            splitChapterRefs.whereType<EpubChapterSplitRef>().take(3).toList();
        ...
        for (final splitChapter in splitChapters) {
          ...
          print(
              '    Part ${splitChapter.partNumber} of ${splitChapter.totalParts}');
          ...
          try {
            ...
            if (cfi != null) {
              ...
              print(
                  '    CFI navigation: ${navigationSuccess ? "Success" : "Failed"}');

              if (navigationSuccess) {
                print(
                    '    Target chapter: "${location!.chapterRef.title ?? location.chapterRef.contentFileName}"');
              }
    ...
    group('✂️ Split-Specific CFI Operations', () {
      ...
      test('Character Offset Mapping in Split Chapters', () async {
        ...
        final splitChapterRefs =
            await splitBookRef.getChapterRefsWithSplitting();
        final splitChapters =
            splitChapterRefs.whereType<EpubChapterSplitRef>().take(2).toList();
        ...
        for (final splitChapter in splitChapters) {
          ...
          try {
            ...
            if (splitChapter.partNumber > 1) {
              expect(startOffset,
                  greaterThan(0)); // Not first part, should start after 0
            }
        ...
        for (final splitChapter in splitChapters) {
          ...
          try {
            ...
            expect(
                wordCount,
                lessThan(
                    50000)); // Sanity check - split parts should be smaller
          } catch (e) {
    ...
    group('⚡ Performance and Edge Cases', () {
      ...
      test('CFI Operations Performance with Split Chapters', () async {
        ...
        print(
            '  Completed $operations operations in ${stopwatch.elapsedMilliseconds}ms');
    ...
    group('📋 Split CFI Format Testing', () {
      test('Split CFI Format Creation and Parsing', () async {
        ...
        final splitFromString =
            SplitCFI('epubcfi(/6/4!/split=1,total=2/4/10/2:20)');
      ...
      test('Split CFI Comparison and Ordering', () async {
        ...
        expect(
            cfi1.compare(standardCFI), greaterThan(0)); // Split after standard
