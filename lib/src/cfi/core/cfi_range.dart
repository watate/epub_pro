...
class CFIRange {
  ...
  static CFI fromStartEnd(CFI start, CFI end) {
    ...
    final commonParent =
        _findCommonParent(start.structure.start, end.structure.start);
  ...
  static List<CFI>? splitAt(CFI rangeCFI, CFI splitPoint) {
    if (!rangeCFI.isRange || splitPoint.isRange) {
      throw ArgumentError(
          'Range CFI must be a range and split point must be a point');
    }
  ...
  static List<CFIPart> _findCommonParent(CFIPath path1, CFIPath path2) {
    ...
    for (int i = 0; i < minLength - 1; i++) {
      // -1 to exclude the final positioning part
  ...
  static List<CFIPart> _getRelativePath(
      CFIPath fullPath, List<CFIPart> commonParent) {
