...
class HTMLNavigator {
    ...
    while (currentNode.parentNode != null) {
      ...
      for (int i = 0; i < indexedNodes.length; i++) {
        ...
        if (node == currentNode) {
          ...
          nodeIndex = i;
          adjustedOffset = _calculateOffsetInTextGroup(
              node.cast<DOMNode>(), currentNode, offset);
          break;
        }
      }
      ...
      final part = CFIPart(
        ...
        id: currentNode.id,
        offset:
            currentNode.nodeType == DOMNodeType.text ? adjustedOffset : null,
      );
  ...
  static int? _calculateOffsetInTextGroup(
      List<DOMNode> textGroup, DOMNode targetNode, int? targetOffset) {
    int totalOffset = 0;
  ...
  static CFIStructure createStructureFromRange(DOMRange range) {
    ...
    for (int i = 0; i < minLength - 1; i++) {
      // -1 to exclude final positioning
    ...
    final startRelativeParts =
        startPath.parts.skip(commonParentParts.length).toList();
    final endRelativeParts =
        endPath.parts.skip(commonParentParts.length).toList();

    return CFIStructure(
      parent: commonParentParts.isNotEmpty
          ? CFIPath(parts: commonParentParts)
          : null,
  ...
  static String extractTextBetweenPositions(
      DOMPosition start, DOMPosition end) {
  ...
  static String _extractTextFromComplexRange(
      DOMPosition start, DOMPosition end) {
