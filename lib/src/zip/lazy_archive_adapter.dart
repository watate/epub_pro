...
class LazyArchiveAdapter extends Archive {
  ...
  static Future<LazyArchiveAdapter> fromBytes(List<int> bytes) async {
    final lazyArchive =
        await LazyZipArchive.fromBytes(Uint8List.fromList(bytes));
  ...
  Future<void> clear() async {
    throw UnsupportedError(
        'Clearing files not supported in lazy read-only mode');
  }
  ...
  Future<void> preloadCriticalFiles() async {
    ...
    final criticalFiles = _lazyFiles.where(
        (file) => criticalExtensions.contains(file.extension.toLowerCase()));
  ...
  Future<void> preloadFiles(List<String> fileNames) async {
    final filesToLoad =
        _lazyFiles.where((file) => fileNames.contains(file.name));
  ...
  Map<String, dynamic> getMemoryStats() {
    ...
    return {
      ...
      'memoryEfficiency':
          totalFiles > 0 ? (1 - loadedFiles / totalFiles) * 100 : 0,
    };
  }
