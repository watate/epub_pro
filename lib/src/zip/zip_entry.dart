...
class ZipEntry {
  ...
  Future<Uint8List> readFromFile(RandomAccessFile file) async {
    ...
    await file.setPosition(localHeaderOffset + 30 + fileNameLength + extraFieldLength);
  ...
  Future<Uint8List> readFromBytes(Uint8List data) async {
    ...
    final dataOffset = localHeaderOffset + 30 + fileNameLength + extraFieldLength;
    ...
    final compressedData = data.sublist(dataOffset, dataOffset + compressedSize);
  ...
  Future<Uint8List> _decompressData(Uint8List compressedData) async {
    switch (compressionMethod) {
      ...
      default:
        throw Exception('Unsupported compression method $compressionMethod for file: $fileName');
    }
  }
