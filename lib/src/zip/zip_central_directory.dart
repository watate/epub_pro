...
class ZipCentralDirectory {
  ...
  static Future<Map<String, dynamic>?> _findEndOfCentralDirectory(RandomAccessFile file) async {
    final fileLength = await file.length();
    
    // Search backwards from end of file for EOCD signature
    const maxCommentLength = 65535;
    final searchLength = (fileLength < maxCommentLength) ? fileLength : maxCommentLength + 22;
    ...
    for (var i = searchData.length - 22; i >= 0; i--) {
      if (_readUint32(searchData, i) == 0x06054b50) {
        ...
        if (commentLength > 0 && i + 22 + commentLength <= searchData.length) {
          comment = utf8.decode(searchData.sublist(i + 22, i + 22 + commentLength));
        }
  ...
  static Map<String, dynamic>? _findEndOfCentralDirectoryInBytes(Uint8List data) {
  ...
  static Future<ZipEntry?> _readCentralDirectoryEntry(RandomAccessFile file) async {
    ...
    final extraField = extraFieldLength > 0 ? await file.read(extraFieldLength) : <int>[];
    final comment = commentLength > 0 ? utf8.decode(await file.read(commentLength)) : '';
  ...
  static Map<String, dynamic>? _readCentralDirectoryEntryFromBytes(Uint8List data, int offset) {
    ...
    if (dataOffset + fileNameLength + extraFieldLength + commentLength > data.length) {
      return null;
    }
    
    final fileName = utf8.decode(data.sublist(dataOffset, dataOffset + fileNameLength));
    final extraField = extraFieldLength > 0 
        ? data.sublist(dataOffset + fileNameLength, dataOffset + fileNameLength + extraFieldLength)
        : <int>[];
    final comment = commentLength > 0 
        ? utf8.decode(data.sublist(dataOffset + fileNameLength + extraFieldLength, 
