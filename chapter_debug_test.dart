...
void main() async {
  ...
  final chaptersToDebug = [
    6,
    7,
    8
  ]; // Add chapter numbers you want to debug here
  ...
  try {
    ...
    print(
        'Total chapters found: ${book.chapters.length} (${flatChapters.length} including subchapters)\n');
    ...
    for (int i = 0; i < flatChapters.length; i++) {
      ...
      print(
          '$indent${i + 1}. "${chapter.title}" -> ${chapter.contentFileName} ($path)');
    }
      ...
      if (chapterNum > 0 && chapterNum <= flatChapters.length) {
        ...
        try {
          ...
          if (words.length > 3000) {
            print(
                '   ⚠️  This chapter would be SPLIT (>${words.length} words > 3000 threshold)');
          }
        } catch (e) {
        ...
        print(
            '❌ Chapter $chapterNum not found (valid range: 1-${flatChapters.length})');
      }
    }
  } catch (e) {
